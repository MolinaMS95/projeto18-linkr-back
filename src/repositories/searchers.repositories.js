...
export function getUsersByInitial(requesterid, initial) {
    return connectionDB.query(`
        ...
        FROM (
            ...
            FROM (
                SELECT TRUE AS follows, users.id, users.username, users.pictureurl
                FROM users
            ) u1
            FULL JOIN relationships ON u1.id = relationships.followedid
            WHERE u1.username LIKE '%${initial}%' AND relationships.followerid = $1
        ) f
        FULL JOIN (
            SELECT 'key' AS key, JSON_AGG(u2) AS reminder
            FROM (
                SELECT FALSE AS follows, users.id, users.username, users.pictureurl
                FROM users
